        ...
        public BaseImagePredictor(string modelPath, Accelerator accelerator, bool processAccelerator=false)
        {
          ...
          //  accelerator = Accelerator.NNAPI;
            ...
            try
            {
                try
                {
                //    if (modelPath.Contains("movenet") || modelPath.Contains("age")) throw new System.Exception(modelPath);

                    interpreter = new Interpreter(FileUtil.LoadFile(modelPath), options);
                }
                catch (System.Exception)
                {
                    var path = modelPath.Split(".")[0];
                    Debug.Log($">>>>>  TRYING for {path}");
                    byte[] m_bytes = Resources.Load<TextAsset>(path).bytes;
                    interpreter = new Interpreter(m_bytes, options);
                }
            }
